...
public class PlayerMove : MonoBehaviour 
{
	...
	public float maxSpeed = 6;								//maximum speed of movement in X/Z axis
	...
	private Vector3 screenMovementForward, screenMovementRight, movingObjSpeed;
		...
		foreach (Transform check in floorCheckers)
		{
			...
			if(Physics.Raycast(check.position, Vector3.down, out hit, dist + 0.05f))
			{
				if(!hit.transform.GetComponent<Collider>().isTrigger)
				{
					...
					{
						enemyAI = hit.transform.GetComponent<SimpleEnemies>();
						enemyAI.BouncedOn();
						onEnemyBounce ++;
...
//	}
	...
	public void Charge(float h)
	{        
        //controls an acceleration from normal running speed to charging speed;
        if (h != 0.0f | Input.GetKey(KeyCode.C))
        {
            if (momentum <= 1.2)
            {
        ...
        if (momentum >= 1.2f)
        {
            hazard.collisionEnter = true;
        }
        else
        {
            hazard.collisionEnter = false;
        }
    }
}